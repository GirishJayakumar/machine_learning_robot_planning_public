...
class ConfigsGenerator:
    def __init__(self, config_data=None, batch_experiments_sizes=None, parameter_ranges=None, template_config=None):
    ...
    def initialize_from_config(self, config_data, section_name):
        if config_data.has_option(section_name, 'batch_experiments_sizes'):
            self.batch_experiments_sizes = np.asarray(
                ast.literal_eval(config_data.get(section_name, 'batch_experiments_sizes')), dtype=np.int)
        if config_data.has_option(section_name, 'parameter_ranges'):
            self.parameter_ranges = np.asarray(
                ast.literal_eval(config_data.get(section_name, 'parameter_ranges')), dtype=np.float64)
    ...
    def generate_parameters_sets(self, parameters_values, parameters_sets, layer_idx, para_set):
        ...
        for i in range(len(parameters_values[layer_idx])):
            ...
            if layer_idx == len(parameters_values) - 1:
                ...
                self.generate_parameters_sets(parameters_values, parameters_sets, layer_idx + 1, para_set)
            para_set.pop()
...
class MPPIConfigsGenerator(ConfigsGenerator):
    ...
    def generate_batch_experiment_configs(self, template_config=None):
        ...
        for set in parameters_sets:
            ...
            new_config = copy.copy(template_config)
            ...
            new_config['my_collision_checker1']['obstacles_radius'] = str(obstacle_radius)
            new_config['my_controller1']['inverse_temperature'] = str(inverse_temperature)
            new_config['my_stochastic_trajectories_sampler1']['uncontrolled_trajectories_portion'] = str(uncontrolled_trajectories_portion)
            new_config['my_stochastic_trajectories_sampler1']['number_of_trajectories'] = str(number_of_trajectories)
            experiment_name = 'exp_obs1x{}_obs1y{}_obs1r{}_obs2x{}_obs2y{}_obs2r{}_gamma{}_alpha{}_Numtraj{}_MPPI'.format(
                set[0], set[1], set[2], set[3], set[4], set[5], set[6], set[7],
                set[8])  # TODO: you can extend this to adjust more parameters
            new_config['logger']['experiment_name'] = str(experiment_name)
            experiment_config_name = experiment_name + '.cfg'
            with open('configs/batch_configs/'+experiment_config_name,
                      'w') as configfile:
                new_config.write(configfile)
            experiment_names.append(experiment_name)

        self.create_run_batch_MPPI_config_file(parameters_sets, experiment_names)

    def create_run_batch_MPPI_config_file(self, parameters_sets, experiment_names):
        run_batch_MPPI_config = ConfigParser.ConfigParser()
        run_batch_MPPI_config['run_batch_MPPI'] = {}
        run_batch_MPPI_config['run_batch_MPPI']['experiment_names'] = '['
        for experiment_name in experiment_names:
            if run_batch_MPPI_config['run_batch_MPPI']['experiment_names'] != '[':
                run_batch_MPPI_config['run_batch_MPPI']['experiment_names'] += ', '
            run_batch_MPPI_config['run_batch_MPPI']['experiment_names'] += '\'' + experiment_name + '\''
        run_batch_MPPI_config['run_batch_MPPI']['experiment_names'] += ']'
...
class AutorallyCSSMPCConfigsGenerator(ConfigsGenerator):
    ...
    def generate_batch_experiment_configs(self, template_config=None):
        ...
        for set in parameters_sets:
            ...
            experiment_name = 'exp_tB{}_tC{}_tD{}_kS{}_cS{}_tf{}_Autorally_CSSMPC'.format(
                tire_B, tire_C, tire_D, kSteering, cSteering, throttle_factor)  # TODO: you can extend this to adjust more parameters
            new_config['logger']['experiment_name'] = str(experiment_name)
            experiment_config_name = experiment_name + '.cfg'
            with open('configs/batch_configs/'+experiment_config_name,
                      'w') as configfile:
        ...
        self.create_run_batch_Autorally_CSSMPC_config_file(parameters_sets, experiment_names)

    def create_run_batch_Autorally_CSSMPC_config_file(self, parameters_sets, experiment_names):
        run_batch_MPPI_config = ConfigParser.ConfigParser()
        run_batch_Autorally_CSSMPC_section_name = 'run_batch_Autorally_CSSMPC'
        run_batch_MPPI_config[run_batch_Autorally_CSSMPC_section_name] = {}
        run_batch_MPPI_config[run_batch_Autorally_CSSMPC_section_name]['experiment_names'] = '['
        for experiment_name in experiment_names:
            if run_batch_MPPI_config[run_batch_Autorally_CSSMPC_section_name]['experiment_names'] != '[':
                run_batch_MPPI_config[run_batch_Autorally_CSSMPC_section_name]['experiment_names'] += ', '
            run_batch_MPPI_config[run_batch_Autorally_CSSMPC_section_name]['experiment_names'] += '\'' + experiment_name + '\''
        run_batch_MPPI_config[run_batch_Autorally_CSSMPC_section_name]['experiment_names'] += ']'
        with open('configs/run_batch_Autorally_CSSMPC.cfg', 'w') as configfile:
            run_batch_MPPI_config.write(configfile)
            configfile.close()