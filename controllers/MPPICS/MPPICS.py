...
class MPPICS(MpcController):
    ...
    def initialize_from_config(self, config_data, section_name):
        ...
        MPPI_stochastic_trajectories_sampler_section_name = config_data.get(section_name, 'stochastic_trajectories_sampler_1')
        self.MPPI_stochastic_trajectories_sampler = factory_from_config(stochastic_trajectories_sampler_factory_base, config_data, MPPI_stochastic_trajectories_sampler_section_name)
        covariance_steering_stochastic_trajectories_sampler_section_name = config_data.get(section_name, 'stochastic_trajectories_sampler_2')
        self.covariance_steering_stochastic_trajectories_sampler = factory_from_config(stochastic_trajectories_sampler_factory_base, config_data, covariance_steering_stochastic_trajectories_sampler_section_name)

    def plan(self, state_cur):
        # Use MPPI to obtain optimal nominal control commands
        v = copy.deepcopy(self.initial_control_sequence)
        trajectories, us, costs = self.MPPI_stochastic_trajectories_sampler.sample(state_cur, v, self.get_control_horizon(), self.get_control_dim(), self.get_dynamics(), self.get_cost_evaluator())
        beta = np.min(costs)
        ...
        # Use covariance steering to deal with noises
        optimal_trajectory_with_covariance_control, us, costs = self.covariance_steering_stochastic_trajectories_sampler.sample(state_cur, v, self.get_control_horizon(), self.get_control_dim(), self.get_dynamics(), self.get_cost_evaluator())
